...
public class FileDAO {
		...
		public void insertBoard(BoardBean bb){
			...
			try {
				...
				sql = "insert into file_board(bno, id, name, passwd, subject, content,"
		...
		public int getBoardCount(){
			...
			try {
				...
				sql = "select count(*) from file_board";
				pstmt = con.prepareStatement(sql);
				rs = pstmt.executeQuery();
		...
		public ArrayList getBoardList(int startRow, int pageSize){
			...
			try {
				...
				sql="select * from file_board order by re_ref desc, re_seq asc limit ?,?";
		...
		public void updateReadCount(int bno){
			...
			try {
				con = getCon();
				sql = "update file_board set readcount=readcount+1 where bno=?";
				pstmt = con.prepareStatement(sql);
				pstmt.setInt(1, bno);
				pstmt.executeUpdate();
			} catch (Exception e) {
		...
		public BoardBean getBoard(int bno){
			...
			try {
				con = getCon();
				sql = "select * from file_board where bno = ?";
				pstmt = con.prepareStatement(sql);
				pstmt.setInt(1, bno);
				rs = pstmt.executeQuery();
		...
		public int deleteBoard(int bno, String passwd){
			...
			try {
				con = getCon();
				sql = "select passwd from file_board where bno=?";
				pstmt = con.prepareStatement(sql);
				pstmt.setInt(1, bno);
				rs = pstmt.executeQuery();
				...
				if(rs.next()){
					if(rs.getString("passwd").equals(passwd)){
						sql = "delete from file_board where bno=?";
						pstmt = con.prepareStatement(sql);
						pstmt.setInt(1, bno);
						pstmt.executeUpdate();
			...
			try {
				...
				if(rs.next()){
					if(bb.getPasswd().equals(rs.getString("passwd"))){
						...
						if(bb.getFile()!=null){
							sql = "update file_board set name=?, subject=?, content=?, file=? where bno=?";
							pstmt = con.prepareStatement(sql);
							...
							sql = "update file_board set name=?, subject=?, content=? where bno=?";
							pstmt = con.prepareStatement(sql);
							pstmt.setString(1, bb.getName());
							pstmt.setString(2, bb.getSubject());
							pstmt.setString(3, bb.getContent());
